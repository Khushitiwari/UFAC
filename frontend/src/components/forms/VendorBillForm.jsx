...
import { createVendorBillSchema } from '../../validators/vendorBill.schema.js';
import { emptyLine, linesTotal, parseFormDate, toDateInput } from '../../utils/formHelpers.js';
import { formatCurrency } from '../../utils/format.js';
import Button from '../common/Button.jsx';

const VendorBillForm = ({ initialValues, onSubmit, onCancel, contacts = [], products = [], submitLabel = 'Save' }) => {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      const payload = {
        ...
        lines: form.lines.map((l) => ({ ...l, quantity: Number(l.quantity), unitPrice: Number(l.unitPrice) })),
      };
      const result = createVendorBillSchema.safeParse(payload);
      if (!result.success) {
        const fieldErrors = {};
