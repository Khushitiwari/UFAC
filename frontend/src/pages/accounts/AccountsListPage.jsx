...
const AccountsListPage = () => {
  ...
  const { items, meta, loading, refreshing, error, refetch, page, nextPage, prevPage } = useAccounts();
  ...
  return (
    <PageShell title="Chart of Accounts" actions={write ? <Button onClick={() => setModalOpen(true)}>+ New Account</Button> : null}>
      ...
      <Table loading={loading} refreshing={refreshing} columns={columns} data={items} onRowClick={(r) => navigate(`/accounts/${r.id}`)} />
